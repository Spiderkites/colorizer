{block name='layout-header'}
    ...
    <head>
        ...
        <link rel="preload" href="{$ShopURL}/{$templateDir}themes/base/fonts/montserrat/montserrat-v15-latin-200.woff2" as="font" crossorigin/>
            ...
            <header class="d-print-none {if !$isMobile || $Einstellungen.template.theme.mobile_search_type !== 'fixed'}sticky-top{/if} fixed-navbar" id="jtl-nav-wrapper">
                {block name='layout-header-container-inner'}
                    ...
                    {block name='layout-header-category-nav'}
                        ...
                        {navbar toggleable=true fill=true type="expand-lg" style="flex-flow: wrap;" class="justify-content-start {if $nSeitenTyp === $smarty.const.PAGE_BESTELLVORGANG}align-items-center-util{else}align-items-lg-end{/if}"}
